...
class Config:
    ...
    def direct_editor(self, config_file_path, options_file_path):
        ...
        while True:
            ...
            try:
                ...
                if config_file_path:
                    with open(config_file_path, "r", encoding="utf-8") as config_f:
                        config = json.load(config_f)        

                if options_file_path:
                    with open(options_file_path, "r", encoding="utf-8") as options_f:
                        options = json.load(options_f)        
                ...
                if str(index).strip() == 'q':
                    ...
                    config.update(self.config)
                    self.response.print(f"Saving config to {config_file_path}", 'i')
